...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		lblNewLabel_4.setBounds(331, 62, 305, 42);
		...
		btnbuscar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (txtBuscar.getText().equals("")) {
					...
					listarTablaReservasId();
					listarTablaHuespedesId();
				}
			}
		});
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea descartar la reserva?");
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea descartar el registro del huesped?");
					if (confirmar == JOptionPane.YES_OPTION) {
						String valor = tbHuespedes.getValueAt(filaHuespedes, 0).toString();
						huespedController.eliminar(Integer.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "¡Registro del huesped eliminado con éxito!");
						limpiarTabla();
						listarHuespedes();
						listarTablaHuespedes();
					}
				} else {
					JOptionPane.showMessageDialog(null, "Error al eliminar en el registro");
				}
			}
		});
	...
	private void listarTablaReservasId() {
		List<Reserva> reserva = listarIdReservas();
		try {
			for (Reserva reservas : reserva) {
				modelo.addRow(new Object[] {reservas.getId(), reservas.getFechaEntrada(), reservas.getFechaSalida(),
						reservas.getValor(), reservas.getFormaPago() });
		...
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresent(fila -> {
					...
					if (tbReservas.getSelectedColumn() == 0) {
						JOptionPane.showMessageDialog(this, "No es posible cambiar los ID");
					} else {
						this.reservaController.modificar(fechaEntrada, fechaSalida, valor, formaPago, id);
	...
	private void listarTablaHuespedesId() {
		List<Huesped> huesped = listarIdHuespedes();
		modeloHuesped.setRowCount(0);
		try {
			for (Huesped huespedes : huesped) {
				modeloHuesped.addRow(new Object[] {huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(),
		...
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
				.ifPresentOrElse(fila -> {
					...
					if(tbHuespedes.getSelectedColumn() == 0 || tbHuespedes.getSelectedColumn() == 6) {
						...
						this.huespedController.modificar(id, nombre, apellido, fechaNacimiento, nacionalidad, telefono, idReserva);
						JOptionPane.showMessageDialog(this, String.format("Registro modificado con éxito"));
					}

				}, () -> JOptionPane.showMessageDialog(this, "No es posible realizar los cambios"));
